...
class Args:
    ...
    ont_mode: str = "task"
    """the mode of the continual learning task, `dynamics` or `task`"""
    num_rounds: int = 10
...
"""
    ...
    """------------------------Plasticine------------------------"""
    # env setup
    env_ids = ['humanoid_stand', 'humanoid_walk', 'humanoid_run', 'humanoid_stand', 'humanoid_walk', 'humanoid_run']
    envs = ContinualDMC(
        env_ids=env_ids,
    ...
    for global_step in range(args.total_timesteps):
        """------------------------Plasticine------------------------"""
        if global_step % args.num_steps_per_round == 0 and global_step > 0:
