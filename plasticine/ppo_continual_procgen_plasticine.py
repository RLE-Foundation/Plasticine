...
class Args:
    ...
    """the number of tasks"""
    task_timesteps: int = int(2e6)
    """the timesteps for each task"""
    level_steps: int = 20
    """the level steps for task switching"""
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"ppo_continual_procgen_runs/{run_name}")
    writer.add_text(
        "hyperparameters",
    ...
    # env setup
    envs = ContinualProcgenIntraTask(
        env_name=args.env_id, 
        num_envs=args.num_envs, 
        start_level=0, 
        max_levels=args.number_of_tasks, 
    ...
    if args.use_trac_optimizer:
        optimizer = start_trac(f'ppo_continual_procgen_runs/{run_name}/trac.text', torch.optim.Adam)(agent.parameters(), lr=args.learning_rate)
    ...
    for task_id in range(1, args.number_of_tasks + 1):
        ...
        for iteration in range(1, args.num_iterations + 1):
            for step in range(0, args.num_steps):
                ...
                for item in info:
                    if "episode" in item.keys():
                        print(f"level={envs.get_current_level()}, global_step={global_step}, episodic_return={item['episode']['r']}")
                        writer.add_scalar("charts/episodic_return", item["episode"]["r"], global_step)
                        writer.add_scalar("charts/episodic_length", item["episode"]["l"], global_step)
                        writer.add_scalar("charts/current_level", envs.get_current_level(), global_step)
                        break
            ...
            if iteration % 10 == 0:
                ...
                with torch.no_grad():
                    hidden, _, _ = agent(b_obs)
                dormant_units = compute_dormant_units(agent.encoder, b_obs.permute((0, 3, 1, 2)) / 255.0, agent.af_name, tau=0.025)
        ...
        if args.use_plasticity_injection:
            ...
            agent.plasticine_reset_layers(reset_encoder=False, reset_actor=True, reset_critic=True) # In default, we only reset the final layers of the agent.
        """🎯============================== Plasticine Operations ==============================🎯"""
